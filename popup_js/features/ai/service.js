/**
 * 文件说明：AI 模块服务层。
 * 职责：封装 AI 相关的 runtime 消息、延时与测速文本生成。
 */
...
export async function sendAIMessage(message, options = {}) {
    ...
    const requestPromise = chrome.runtime.sendMessage(message)
        .then((response) => response || {ok: false, error: 'empty-response'})
        .catch((error) => ({
            ok: false,
 ...
 * 构建测速使用的长文本。
 */
export function buildBenchmarkText() {
    const seed = '帝高阳之苗裔兮，朕皇考曰伯庸。 摄提贞于孟陬兮，惟庚寅吾以降。 皇览揆余初度兮，肇锡余以嘉名。 名余曰正则兮，字余曰灵均。 纷吾既有此内美兮，又重之以修能。 扈江离与辟芷兮，纫秋兰以为佩。 汩余若将不及兮，恐年岁之不吾与。 朝搴阰之木兰兮，夕揽洲之宿莽。\n' +
    ...
    const repeated = seed.repeat(1);
    return repeated.substring(0, 1000);
}
